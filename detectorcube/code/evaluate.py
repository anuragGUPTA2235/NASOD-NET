...
def evaluate_model(model: YOLOv1, test_loader: DataLoader) -> Tuple[float, List[float]]:
    ...
    with th.no_grad():
        model.eval()
        for x, bboxes_gt in tqdm(test_loader):
...
def setup_evaluation(genome1:list,genome2:list,genome3:list) -> Tuple[YOLOv1, DataLoader]:
    """
    ...
    model = YOLOv1(S=S,
                   B=B,
                   C=VOC_Detection.C,genome1=genome1,genome2=genome2,genome3=genome3).to(DEVICE)
    model = th.load(TRAINED_MODEL_WEIGHTS)
...
def map_metric(genome1:list,genome2:list,genome3:list):
    archive_file_path = "/run/user/1001/projectredmark/new_nasod/bioex/archive.txt"
    model, test_loader = setup_evaluation(genome1,genome2,genome3)
    mAP, average_precisions = evaluate_model(model, test_loader)
    print(f'Mean Average Precision =',mAP)
    with open(archive_file_path,'a') as file:
      file.write(f"genome1:{genome1} genome2:{genome2} genome3:{genome3} mAP:{mAP}")
      file.write("\n")
