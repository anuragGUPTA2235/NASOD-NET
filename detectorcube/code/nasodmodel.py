...
class YOLOv1(nn.Module):
    def __init__(self, S: int, B: int, C: int, genome1 : list,genome2 : list,genome3 : list,mode: Optional[str] = 'detection') -> None:
        ...
        self.backbone = CombinedNet(256,256,91,genome1,genome2,genome3).cuda()
        if mode == 'detection':
            in_channels=1024
...
genome2 = [6,5,4,3,0,0]
genome3 = [6,5,4,3,2,1]     
model = YOLOv1(S=7, B=2, C=91,genome1=genome1,genome2=genome2,genome3=genome3)
model = model.cuda()
x = th.randn(1, 3, 448, 448).cuda()
y = model(x)
print(y.shape)
